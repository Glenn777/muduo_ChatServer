...
// 聊天客户端程序实现，main线程用户发送线程， 子线程用户接收线程
    ...
    for(;;)
    {
        ...
        switch (choice)
        {
            case 1 : // login 业务
            {
                ...
                {
                    char buffer[1024] = {0};
                    len =recv(clientfd, buffer, 1024, 0); // 等待响应
                    if(-1 == len)
                    {
                    ...
                    else
                    {
                        ...
                        else // 登录成功
                        {
                            ...
                            if (responsejs.contains("groups"))
                            {
                                ...
                                for(string &gourpstr : vec1)
                                {
                                    json grpjs = json::parse(gourpstr);
                                    Group group;
                            ...
                            if(responsejs.contains("offlinemsg"))
                            {
                                ...
                                for (string &str : vec)
                                {
                                    ...
                                    {
                                        cout << "群消息[" << js["groupid"].get<int>()<<"]: "<<js["time"].get<string>()<<" [" <<js["id"]<<"]" <<js["name"].get<string>()<<" said: "<< js["msg"].get<string>()<<endl;
                                        continue;
                                    }
                                }
                            }
